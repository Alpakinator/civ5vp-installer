...
**Version** - the ref of the Community-Patch-DLL repository being installed. Three selectable tiers: a **Release** (a `Release-*` tag), the **Latest Development Version** (upstream `master` HEAD), or an **Arbitrary Ref** (any branch/tag/commit, advanced users only). A Local Repo's version is whatever its working tree currently contains. Ticket 13 added a fourth tier, the **Unofficial Build**: one commit after the newest Release, labelled `X.Y.Z.NN` by its position (`5.4.3.07` is the seventh change since `Release-5.4.3`), offered by the picker's unofficial-versions toggle and pinned to the commit hash it named when listed. The toggle lists the changes since the *release before last*, not only those after the newest one, so it spans two Releases: `5.4.3.01…` are the changes that became `Release-5.4.4`, `5.4.4.01…` the ones since. A Release commit is never listed as an Unofficial Build - the picker offers it as a Release. The Latest Development Version is no longer offered by the picker, nor shown by it - the newest Unofficial Build is that commit, with an honest name - and a settings file that still names it opens on the newest Release instead.
...
**43 Civs** - a toggle, legal with both Flavors and with or without EUI. Means: the DLL placed in `(1)` (the only DLL deployed) is the 43-civ one - the Built DLL compiled with the 43-civ setting, or `(3b)`'s Shipped DLL where that is current - and the `(3b)` mod folder is deployed containing only its `.modinfo` and `AdvancedSetup.lua`.
...
**Built DLL** - `CvGameCore_Expansion2.dll` compiled locally by the installer with clang. Every Version but one gets it: unofficial builds, `master`, any ref typed in, and every Local Repo, because outside a Release commit the DLL checked into the repository is older than the sources beside it.

**Shipped DLL** - the `CvGameCore_Expansion2.dll` checked into the Installation Source, deployed as-is instead of compiling one. Legal only where it is **current**: the commit being installed is the commit that last changed that file, which upstream arranges by refreshing both checked-in DLLs in the Release commit itself. Checked against the repository, never assumed from the tag's name, so an Arbitrary Ref naming a Release commit qualifies and a Release that forgot to refresh its DLL does not; when that check cannot be made at all, "a `Release-*` tag points at this commit" stands in for it. `(1)`'s copy for an ordinary install, `(3b)`'s for a 43-Civs one; either way it is deployed into `(1)`, where the Built DLL would have gone. A Release with no Shipped DLL for the configuration is compiled instead. The player can decline it - "compile the DLL myself" - and the choice is part of the Build Fingerprint (ADR-0001's correction).
...
**Toolchain** - everything needed to produce the Built DLL: portable clang/lld and the extracted Windows SDK 7.0 + VC9 CRT. Acquired by **Toolchain Bootstrap**: downloaded and unpacked on first build, then kept in the **Toolchain Cache**. Never bundled inside the installer executable, and never fetched for an install that compiles nothing - a Release on the stock Lua engine takes its Shipped DLL and needs no Toolchain at all.
...
**Build Fingerprint** - a hash of everything that determines the DLL a Deployment installs: all source inputs at the selected Version (or the Local Repo's working files), compiler flags (including any `dll-flags.txt` optimisation override), the 43-Civs setting, the Toolchain version, and whether the DLL was built or is the Shipped one. Recorded next to the deployed DLL together with the DLL's own hash; when both still match, the build is skipped.
...
**Deployment** - copying the selected files from the Installation Source into the MODS and DLC Folders according to the Install Configuration, with the standard exclusions (project files, source art, docs, checked-in DLLs). A checked-in DLL is excluded from every folder copy even when it is the Shipped DLL: which DLL a configuration installs is decided once, and Sync places that one file into `(1)` at the end.
