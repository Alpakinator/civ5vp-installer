...
//! wine — ADR-0001), applies the Linux case-folding fix-ups, and leaves the result
...
//! keep having none, while this needs an HTTP client and three archive parsers.
...
// This is reachable from the UI, through the `ToolchainRunner` boundary, so nothing here may
// panic. Unlike the Core, this crate's tests live *inside* it — the parsers are internal and
// there is no public API to reach them through — so each `mod tests` carries the `allow`
// tests are granted explicitly. Nothing outside a `#[cfg(test)]` module may.
#![deny(
    clippy::unwrap_used,
    clippy::expect_used,
    clippy::panic,
