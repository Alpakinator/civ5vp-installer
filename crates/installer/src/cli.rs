...
OPTIONS:
    ...
    --size <WxH>         Window size in points, e.g. 900x900. Repeat for several sizes.
                         Default: 900x900
...
pub const DEFAULT_SIZE: [f32; 2] = [900.0, 900.0];
    ...
    while let Some(argument) = args.next() {
        match argument.as_str() {
            ...
            "--size" => {
                let value = args
                    .next()
                    .ok_or_else(|| "--size needs a value like 900x900".to_owned())?;
...
fn parse_size(value: &str) -> Result<[f32; 2], String> {
    let (width, height) = value
        .split_once(['x', 'X'])
        .ok_or_else(|| format!("expected a size like 900x900, got {value}"))?;
    let width: f32 = width
        .trim()
        .parse()
